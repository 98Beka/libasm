...
/*   Updated: 2021/02/13 09:26:40 by ehande           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	wk(fd, !ft_strncmp(s1, s2, larger_len(s1, s2)), 0x000001);
	w(fd, "\n\n\n");
	free(s1);
	free(s2);
	return(0);
...
void ch_strlen(int fd, char *v)
{
	...
	w(fd, "\nmy: ");
	w(fd, s2);
	wk(fd, !ft_strncmp(s1, s2, larger_len(s1, s2)), 0b000010);
	w(fd, "\n\n\n");
	free(s1);
	free(s2);
}
...
void ch_strdup(int fd, char *v)
{
	...
	w(fd, "\nmy: ");
	w(fd,(s2 = ft_strdup(v)));
	wk(fd, !ft_strncmp(s1, s2, larger_len(s1, s2)), 0b000100);
	w(fd, "\n\n\n");
	free(s1);
	free(s2);
}
...
void ch_strcpy(int fd, char *v)
{
	...
	wk(fd, !ft_strncmp(s1, s2, larger_len(s1, s2)), 0b001000);
	w(fd, "\n\n\n");
	free(s1);
	free(s2);
}
